...
func TestPartitionStreams(t *testing.T) {
	...
	d = <-preprocesser.TfOut
	testFile2 := []string{"i", "like", "banana", "sandwich", "cats", "are", "fun", "yummy", "yummy", "yummy", "the", "cat", "in", "the", "hat", "ate", "a", "yummy", "banana", "and", "didnt", "like", "it"}
	for i, token := range d {
		if token != testFile2[i] {
			t.Error("incorrect input: expected %s but got %s", testFile2[i], token)
		}
	}
